...
# 총 견적 범위 — 중간값(median) 기준 ± 마진으로 통일
# 전체/부분 공통: Σ공종별_중간값 또는 total_costs 중간값 기준으로 범위 산출
# (P25~P75 + 버퍼 방식은 공종 수·데이터 분포에 따라 범위가 2배까지 벌어지는 문제)
전체_LO_MARGIN = 0.20  # 전체 리모델링 하한 마진 (−20%)
전체_HI_MARGIN = 0.46  # 전체 리모델링 상한 마진 (+46%)
부분_LO_MARGIN = 0.22  # 부분 리모델링 하한 마진 (−22%): 공종 합산이라 약간 넓게
부분_HI_MARGIN = 0.25  # 부분 리모델링 상한 마진 (+25%)

# 사례 수 기반 마진 배율 — 사례가 많을수록 신뢰도가 높으므로 마진을 좁힘
# n≥12: 상하한 모두 대폭 축소 → 범위 ~1.38x, coverage ~50%
# n<12: 데이터 희소 구간이므로 보수적으로 유지
...
class EstimateEngine:
    ...
    def cost_range(values, max_ratio: float = None):
        ...
        if max_ratio is not None and lo > 0 and hi > lo * max_ratio:
            hi = int(lo * max_ratio)
        mid = max(lo, min(mid, hi))  # hi 클램프 후 mid도 [lo, hi] 범위로 보정
    ...
    def generate(self, inp: dict) -> dict:
        ...
        if inp.get("시공범위") == "전체" and total_costs:
            ...
            # 공종별 중간값 합산 후 ± 마진으로 범위 산출
            # (Σ최소/Σ최대 방식은 공종 수가 늘수록 범위가 복합적으로 넓어지는 문제)
        ...
        # 선택했지만 단가 범위를 산출하지 못한 공종 (총 견적에는 반영됨)
        데이터_부족_공종 = [
            g for g in 추출대상_공종들
            if g not in 공종별_범위
        ]
